    ...
    methods: {
      ...
      openTages(type,data) {
        if(type == 'add') {
          ...
          console.log(data)
          this.tags_name = data.tags_name;
          this.tags_create_name = data.create_time;
          this.tags_update_name = data.update_time;
          this.id = data.id;
        }
      ...
      add_edit() {
        ...
        this.$http.post(this.publicurl + '/admin/Tags/' + this.fromStatus, {
          'id': this.id,
          'name': this.tags_name,
          'create': this.tags_create,
          'update': this.tages_update
        })
        .then(function (res) {
          if(res.data.code == 1){
              _this.$message({
